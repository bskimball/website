...
The original orchestration path is still useful. The `task` tool launches a fresh Pi process in JSON mode with no restored session. It discovers the selected agent, validates the working directory, injects the specialist prompt, streams events into the terminal UI, and returns the final report to the lead.
...
Some jobs need a longer relationship than one prompt and one report. The newer orchestration path starts Pi in RPC mode with an isolated session directory and exposes a lifecycle instead of a single call:
